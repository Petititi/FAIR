...
EM_modif::EM_modif(int _nclusters, int _covMatType, double initVar1, double initVar2, const TermCriteria& _termCrit)
{
    ...
    this->initVar[0] = initVar1;
    this->initVar[1] = initVar2;
}

EM_modif::EM_modif(int _nclusters, int _covMatType, const TermCriteria& _termCrit)
{
    nclusters = _nclusters;
    covMatType = _covMatType;
    maxIters = (_termCrit.type & TermCriteria::MAX_ITER) ? _termCrit.maxCount : DEFAULT_MAX_ITERS;
...
bool EM_modif::train(InputArray samples,
...
bool EM_modif::trainE(InputArray samples,
                InputArray _means0,
                InputArray _covs0,
...
bool EM_modif::trainM(InputArray samples,
                InputArray _probs0,
                OutputArray logLikelihoods,
                OutputArray labels,
                OutputArray probs)
...
Vec2d EM_modif::predict(InputArray _sample, OutputArray _probs) const
{
                    ...
                    const std::vector<Mat>* covs, const Mat* weights)
{
    ...
    CV_Assert(startStep == EM_modif::START_AUTO_STEP ||
              startStep == EM_modif::START_E_STEP ||
              startStep == EM_modif::START_M_STEP);
    CV_Assert(covMatType == EM_modif::COV_MAT_GENERIC ||
              covMatType == EM_modif::COV_MAT_DIAGONAL ||
              covMatType == EM_modif::COV_MAT_SPHERICAL);
    ...
    if(startStep == EM_modif::START_E_STEP)
    {
        CV_Assert(means);
    }
    else if(startStep == EM_modif::START_M_STEP)
    {
        CV_Assert(probs);
    }
}
...
void EM_modif::setTrainData(int startStep, const Mat& samples,
                      const Mat* probs0,
                      const Mat* means0,
                      ...
                      const Mat* weights0)
{
    ...
    bool isKMeansInit = (startStep == EM_modif::START_AUTO_STEP) || (startStep == EM_modif::START_E_STEP && (covs0 == 0 || weights0 == 0));
    // Set checked data
    preprocessSampleData(samples, trainSamples, isKMeansInit ? CV_32FC1 : CV_64FC1, false);

    // set probs
    if(probs0 && startStep == EM_modif::START_M_STEP)
    {
    ...
    // set weights
    if(weights0 && (startStep == EM_modif::START_E_STEP && covs0))
    {
    ...
    // set means
    if(means0 && (startStep == EM_modif::START_E_STEP/* || startStep == EM_modif::START_AUTO_STEP*/))
        means0->convertTo(means, isKMeansInit ? CV_32FC1 : CV_64FC1);

    // set covs
    if(covs0 && (startStep == EM_modif::START_E_STEP && weights0))
    {
...
void EM_modif::decomposeCovs()
{
    CV_Assert(!covs.empty());
    covsEigenValues.resize(nclusters);
    if(covMatType == EM_modif::COV_MAT_GENERIC)
    ...
    for(int clusterIndex = 0; clusterIndex < nclusters; clusterIndex++)
    {
        ...
        if(covMatType == EM_modif::COV_MAT_SPHERICAL)
        {
            double maxSingularVal = svd.w.at<double>(0);
            covsEigenValues[clusterIndex] = Mat(1, 1, CV_64FC1, Scalar(maxSingularVal));
        }
        else if(covMatType == EM_modif::COV_MAT_DIAGONAL)
        {
            covsEigenValues[clusterIndex] = svd.w;
        }
        else //EM_modif::COV_MAT_GENERIC
        {
            covsEigenValues[clusterIndex] = svd.w;
...
void EM_modif::clusterTrainSamples()
{
    ...
    float *meansVal = meansFlt.ptr<float>(0);
    if( meansVal[0]>meansVal[1] )
    {
      double testTmp;
      CV_SWAP( meansVal[0], meansVal[1], testTmp );
      labels = ( labels - Scalar( 1.0 ) ) * -1.0;
    }
...
void EM_modif::computeLogWeightDivDet()
{
    ...
    for(int clusterIndex = 0; clusterIndex < nclusters; clusterIndex++)
    {
        ...
        for(int di = 0; di < evalCount; di++)
            logDetCov += std::log(covsEigenValues[clusterIndex].at<double>(covMatType != EM_modif::COV_MAT_SPHERICAL ? di : 0));

        logWeightDivDet.at<double>(clusterIndex) = logWeights.at<double>(clusterIndex) - 0.5 * logDetCov;
    }
}

bool EM_modif::doTrain(int startStep, OutputArray logLikelihoods, OutputArray labels, OutputArray probs)
{
    int dim = trainSamples.cols;
    // Precompute the empty initial train data in the cases of EM_modif::START_E_STEP and START_AUTO_STEP
    if(startStep != EM_modif::START_M_STEP)
    {
        if(covs.empty())
        {
...
Vec2d EM_modif::computeProbabilities(const Mat& sample, Mat* probs) const
{
    ...
    for(int clusterIndex = 0; clusterIndex < nclusters; clusterIndex++)
    {
        ...
        Mat rotatedCenteredSample = covMatType != EM_modif::COV_MAT_GENERIC ?
                centeredSample : centeredSample * covsRotateMats[clusterIndex];

        double Lval = 0;
        for(int di = 0; di < dim; di++)
        {
            double w = invCovsEigenValues[clusterIndex].at<double>(covMatType != EM_modif::COV_MAT_SPHERICAL ? di : 0);
...
void EM_modif::mStep()
{
    ...
    for(int clusterIndex = 0; clusterIndex < nclusters; clusterIndex++)
    {
        ...
        Mat clusterCov = covMatType != EM_modif::COV_MAT_GENERIC ?
        ...
        for(int sampleIndex = 0; sampleIndex < trainSamples.rows; sampleIndex++)
        {
            ...
            else
            {
                ...
                for(int di = 0; di < dim; di++ )
                {
                    ...
                    clusterCov.at<double>(covMatType != EM_modif::COV_MAT_SPHERICAL ? di : 0) += p*val*val;
                }
            }
        }
        ...
        */
        clusterCov = Scalar( initVar[clusterIndex] );

        // Update covsRotatEM_modifats for EM_modif::COV_MAT_GENERIC only
        if(covMatType == EM_modif::COV_MAT_GENERIC)
        {
            SVD svd(covs[clusterIndex], SVD::MODIFY_A + SVD::FULL_UV);
            covsEigenValues[clusterIndex] = svd.w;
